...
# Matplotlib ultra-rapide
plt.ion()
fig, ax = plt.subplots(figsize=(6,6))

# ⚠️ Création initiale du scatter : x et y séparés
...
ax.set_xlim(-6, 6)
ax.set_ylim(-6, 6)
ax.set_title('RPLidar 2D – ICP Trajectoire Optimisée')
ax.set_xlabel('X (m)')
ax.set_ylabel('Y (m)')
ax.grid(True)
...
try:
    while True:
        ...
        if scan_skip % SKIP_N != 0 or len(curr_scan) == 0:
            continue  # jette ce scan pour éviter buffer overflow
        ...
        # ----- Mise à jour ultra-rapide Matplotlib -----
